...
# glimpse(add_trn_val_test$`40600`)
# glimpse(one_hot$`40600`)


# create train-val-test for h20
train_tbl <-
  one_hot %>% 
  map(~ (filter(.x,
                data_use_el_stop_id == "01_train"
...
train_h2o <- train_tbl %>% map(~ as.h2o(.x))
valid_h2o <- valid_tbl %>% map(~ as.h2o(.x))
test_h2o  <- test_tbl %>% map(~ as.h2o(.x))
...
# Extract leader model
automl_leader_b <-
  pmap(.l = list(a = automl_models_h2o_b),
       .f = function(a) {
         ldr = a@leader
