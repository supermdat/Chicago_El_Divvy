...
### Based on examples from [http://www.business-science.io/timeseries-analysis/2018/07/01/keras-lstm-sunspots-part2.html](http://www.business-science.io/timeseries-analysis/2018/07/01/keras-lstm-sunspots-part2.html) and [https://tensorflow.rstudio.com/blog/sunspots-lstm.html](https://tensorflow.rstudio.com/blog/sunspots-lstm.html)  
  ...
  Try an example on just the most recent split for one url.
```{r}
...
##### Shaping the data for LSTM modeling
# These variables are being defined just because of the order in which we present things in this post (first the data, then the model). They will be superseded by FLAGS$n_timesteps, FLAGS$batch_size and n_predictions in the following snippet
n_timesteps <- 7
n_predictions <- n_timesteps
batch_size <- 180

# t(sapply(1:(length(train_vals) - (14 + 14) + 1),
#          function(x)
#            train_vals[x:(x + (14 + 14) - 1)]
#          )
#   )
...
###### extract el_rides from data frame
...
# to find the optimal parameter settings, looping through for a make-shift grid-search of parameters

# original settings in sequential loop
  # for (#lrn_rate in seq(0.00, 0.003, 0.001)
  #      lrn_rate in seq(0.00, 0.001, 0.001)
  #      ) {
  #   for #(rec_drpout in seq(0.0, 0.2, 0.1)
  #       (rec_drpout in seq(0.0, 0.1, 0.1)
  #        ) {
  #     for #(drpout in seq(0.0, 0.2, 0.1)
  #         (drpout in seq(0.1, 0.1, 0.1)
  #     ) {
  #       for #(momntm in seq(0.7, 0.9, 0.1)
  #           (momntm in seq(0.9, 0.9, 0.1)
  #       ) {
...
# 4 x 3 x 3 x 3 = 108 different combinations tried
# set the outer-most-loop with lrn_rate as it has the most number of possibilities

keras_parameter_tuning_trials <-
  foreach(lrn_rate = seq(0.00, 0.003, 0.001),
          .packages = c("tfruns", "keras", "tidyverse", "purrr"),
          .combine = c,
          .verbose = TRUE
          ) %:% 
  foreach(rec_drpout = seq(0.0, 0.2, 0.1),
          .packages = c("tfruns", "keras", "tidyverse", "purrr"),
          .combine = c,
          .verbose = TRUE
          ) %:% 
  foreach(drpout = seq(0.0, 0.2, 0.1),
          packages = c("tfruns", "keras", "tidyverse", "purrr"),
          .combine = c,
          .verbose = TRUE
          ) %:% 
  foreach(momntm = seq(0.7, 0.9, 0.1),
          packages = c("tfruns", "keras", "tidyverse", "purrr"),
          .combine = c,
          .verbose = TRUE
          ) %dopar% {
...
FLAGS <- flags(
  ...
  flag_integer("batch_size", 180),
  # size of the hidden state, equals size of predictions
  flag_integer("n_timesteps", 7),
...
# tri_list <- list()
# tri_list[[name]] <- list(model_train = model,
...
mape_train_list <- list()
mape_train_list[[name]] <- mape_train

return(mape_train_list)
    }
...
# saving is done to avoid having to run the models again
saveRDS(keras_parameter_tuning_trials,
        paste0(wd,
               "/Data_Processed/",
               "keras_parameter_tuning_trials.Rds"
               )
        )

saveRDS(runtime_dopar,
        paste0(wd,
               "/Data_Processed/",
               "runtime_dopar.Rds"
               )
...
FLAGS <- flags(
  ...
  flag_boolean("stack_layers", FALSE),
  # number of samples fed to the model in one go
  flag_integer("batch_size", 360),
  # size of the hidden state, equals size of predictions
  flag_integer("n_timesteps", 14),
  # how many epochs to train for
  flag_integer("n_epochs", 100),
  # fraction of the units to drop for the linear transformation of the inputs
  flag_numeric("dropout", best_settings$drpout_value), # 0.1
  # fraction of the units to drop for the linear transformation of the recurrent state
  flag_numeric("recurrent_dropout", best_settings$rec_drpout_value), # 0.0
  # loss function. Found to work better for this specific case than mean squared error
  flag_string("loss", "mape"),
  # optimizer = stochastic gradient descent. Seemed to work better than adam or rmsprop here
  # (as indicated by limited testing)
  ...
  flag_integer("n_units", 128),
  # learning rate
  flag_numeric("lr", best_settings$lrn_rate_value), # 0.003
  # momentum, an additional parameter to the SGD optimizer
  flag_numeric("momentum", best_settings$momtum_value), # 0.8
  # parameter to the early stopping callback
  flag_integer("patience", 10)
)
...
ggplot(compare_test, aes(x = el_date, y = el_rides)) + geom_line() +
...
obtain_predictions <-
  function(split) {
    ##### split into train, validation, and test
    df_trn <- analysis(split) %>% 
      select(date, pageview_sum)
    df_val <- assessment(split) %>% 
      select(date, pageview_sum)
    df_tst <- add_lags$`ttb.gov/foia/frl.shtml` %>% 
      filter(train_test == "test") %>% 
      select(date, pageview_sum)
    
    df <- bind_rows(df_trn %>% add_column(key = "01_training"),
                    df_val %>% add_column(key = "02_validation"),
                    df_tst %>% add_column(key = "03_testing")
                    ) %>% 
    ...
    ##### Center & Scaling the data
    rec_obj <- recipe(pageview_sum ~ ., df) %>% 
      step_sqrt(pageview_sum) %>% 
      step_center(pageview_sum) %>% 
      step_scale(pageview_sum) %>% 
      prep()
    
    df_processed_tbl <- bake(rec_obj, df)
    
    center_history <- rec_obj$steps[[2]]$means["pageview_sum"]
    scale_history  <- rec_obj$steps[[3]]$sds["pageview_sum"]
    
    ###### Build the LSTM Model
    FLAGS <-
      flags(flag_boolean("stateful", TRUE),
            flag_boolean("stack_layers", FALSE),
            flag_integer("batch_size", 360),
            flag_integer("n_timesteps", 14),
            flag_integer("n_epochs", 100),
            flag_numeric("dropout", best_settings$drpout_value), # 0.2
            flag_numeric("recurrent_dropout", best_settings$rec_drpout_value), # 0.2
            flag_string("loss", "mape"),
            flag_string("optimizer_type", "sgd"),
            flag_integer("n_units", 128),
            flag_numeric("lr", best_settings$lrn_rate_value), # 0.003
            flag_numeric("momentum", best_settings$momtum_value), # 0.9
            flag_integer("patience", 10)
            )
    ...
    ###### extract pageview_sums from data frame
    train_vals <- df_processed_tbl %>% 
      filter(key == "01_training") %>%
      select(pageview_sum) %>% 
      pull()
...
# all_split_preds$`ttb.gov/foia/frl.shtml`$predict[[1]]$train
# all_split_preds$`ttb.gov/foia/frl.shtml`$predict[[1]]$test
...
accuracy_stats <-
  function(df, stat_type) {
    ...
    mape = map_dbl(cols,
                   function(col) {
                     data = df %>% 
                       mutate(pct_error = pageview_sum - !!col / pageview_sum * 100
                              )
    ...
    accuracy_stats = list(rmse = rmse,
                          mape = mape
                          )
    
    if(stat_type == "rmse") {
      result = accuracy_stats$rmse
      } else {
        result = accuracy_stats$mape
        }
...
# length(all_split_preds)
# names(all_split_preds)
# 
# length(all_split_preds$`ttb.gov/foia/frl.shtml`)
# names(all_split_preds$`ttb.gov/foia/frl.shtml`)
# length(all_split_preds$`ttb.gov/foia/frl.shtml`$splits)
...
# length(all_split_preds2$`ttb.gov/foia/frl.shtml`$predict[[1]])
length(all_split_preds2$`ttb.gov/foia/frl.shtml`$predict)

all_split_preds_train <-
  pmap(.l = list(a = all_split_preds2),
       .f = function(a) {
         a[seq(1, 27, by = 2), ] # training sets are every other of 28 total sets
         }
       )

# all_split_preds2$`ttb.gov/foia/frl.shtml`$id %>% length()
# all_split_preds_train$`ttb.gov/foia/frl.shtml`$id %>% length()


all_split_preds_test <-
  pmap(.l = list(a = all_split_preds2),
       .f = function(a) {
         a[seq(2, 28, by = 2), ] # testing sets are every other of 28 total sets
         }
       )

# all_split_preds_test$`ttb.gov/foia/frl.shtml`$id %>% length()
# 
# all_split_preds_train$`ttb.gov/foia/frl.shtml`$predict[[1]] %>% 
#     accuracy_stats(stat_type = "rmse")


# all_split_preds_train$`ttb.gov/foia/frl.shtml` %>% 
#   mutate(rmse = map_dbl(predict,
#                         ~ accuracy_stats(df = .x,
#                                        stat_type = "rmse"
#                                        )
#                         ),
#          mape = map_dbl(predict,
#                         ~ accuracy_stats(df = .x,
#                                        stat_type = "mape"
#                                        )
#                         )
...
# user   system  elapsed 
# 4241.814   11.744 4274.215
# ~ 71 min
start <- proc.time()
all_split_rmses_train <-
  pmap(.l = list(a = all_split_preds_train),
       .f = function(a) {
         a %>% 
           mutate(rmse = map_dbl(predict,
                                 ~ accuracy_stats(df = .x,
...
saveRDS(all_split_rmses_train,
        paste0(wd,
               "/Data_Processed/",
               "all_split_rmses_train.Rds"
               )
        )
...
# user   system  elapsed 
# 1031.631    4.362 1040.847  
# ~ 17 min
start <- proc.time()
all_split_rmses_test <-
  pmap(.l = list(a = all_split_preds_test),
       .f = function(a) {
         a %>% 
           mutate(rmse = map_dbl(predict,
                                 ~ accuracy_stats(df = .x,
...
runtime_accuracy_stats_test <- proc.time() - start
runtime_accuracy_stats_test
rm(start)

saveRDS(all_split_rmses_test,
        paste0(wd,
               "/Data_Processed/",
               "all_split_rmses_test.Rds"
               )
        )
