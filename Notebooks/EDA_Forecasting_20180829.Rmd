...
```
  ...
  Now we can check some summary (MAPE) statistics. **NOTE: these are summary statistics based on the data NOT used to build the models.**  
...
forecast_data_list <-
  add_el_stop_id %>% 
  map(~ get_forecast_data(.x)
      )



# munge the resulting data for ploting
...
plot_actuals_vs_models <-
  ...
  map(~ .x %>% 
        ...
        #                   ) +
        scale_x_date(limits = c(max(.$el_date) - (7 * 3),
                                max(.$el_date) + (7 * 1)
                                )
                     ) +
        ggtitle(label = .$el_stop_id) +
