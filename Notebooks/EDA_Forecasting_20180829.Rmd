...
### Based on examples from [http://www.business-science.io/timeseries-analysis/2018/07/01/keras-lstm-sunspots-part2.html](http://www.business-science.io/timeseries-analysis/2018/07/01/keras-lstm-sunspots-part2.html) and [https://tensorflow.rstudio.com/blog/sunspots-lstm.html](https://tensorflow.rstudio.com/blog/sunspots-lstm.html)  
  ...
  First, setup the infrastructure.
```{r}
...
# 3 * 2 * 2 * 3 * 2 = 72 different combinations tried
...
FLAGS <- flags(
  ...
  flag_integer("batch_size", 28),  # 180
...
# FLAGS$dropout             <-    as.numeric(best_settings$drpout_value)
# FLAGS$recurrent_dropout   <-    as.numeric(best_settings$rec_drpout_value)
# FLAGS$lr                  <-    as.numeric(best_settings$lrn_rate_value)
# FLAGS$decay               <-    as.numeric(best_settings$decay_value)
# FLAGS$momentum            <-    as.numeric(best_settings$momtum_value)
...
# all_split_preds$`40600`$predict[[1]]$train
# all_split_preds$`40600`$predict[[1]]$test
...
# length(all_split_preds_unnest$`40600`$predict)
# length(all_split_preds_unnest$`40910`$predict)
...
#          
#          a[seq(1, lngth_tot - 2, by = 3), ] # training sets are every other of 39 total sets
#          }
#        )

# all_split_preds_unnest$`40600`$id %>% length()
# all_split_preds_train$`40600`$id %>% length()
...
all_split_preds_individ <-
  pmap(.l = list(a = all_split_preds_unnest),
       .f = function(a) {
         ...
         # a$keras_predict.train = train_
         # a$keras_perdict.valid = valid_
         # a$keras_predict.test = test_
...
#          lngth_tot = length(a$predict)
#          
#          a[seq(3, lngth_tot, by = 3), ] # testing sets are every other of 26 total sets
#          }
#        )

# all_split_preds_test$`40600`$id %>% length()
 
# all_split_preds_individ$`40600`$train %>% View()
# all_split_preds_individ$`40600`$train$predict[[1]] %>%
#     accuracy_stats(stat_type = "rmse")

# 
# View(all_split_preds_train$`40910`$predict)
# View(all_split_preds_train$`41660`$predict)

# test <-
#   all_split_preds_individ$`40910`$train %>%  
...
# View(test)
# colnames(test)
# View(test %>% select(id, start_date, rmse, mape))

# all_split_preds_individ$`40600`$train %>% colnames()
# all_split_preds_individ$`40600`$valid %>% colnames()
# all_split_preds_individ$`40600`$test %>% colnames()
...
#                             )
#                   ) %>% 
#            # select(el_stop_id, id, start_date, keras_rmse, keras_mape) %>% 
#            select(-predict)
#          }
#        )
# 
...
# rm(start)
# 
# saveRDS(all_split_rmse_test,
#         paste0(wd,
#                "/Models/",
#                "all_split_rmse_test.Rds"
#                )
#         )
