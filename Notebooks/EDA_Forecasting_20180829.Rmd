...
```
  ...
  **To be deleted later (below)**  
  ...
  Just reloading data to save some time.
```{r}
...
### Weather Variables
  ...
  As the prediction forcast is 14 days in the future, I don't think we'll be able to accuratley know the temperature variables `tmin` and `tmax` within one degree of accuracy. So here, I simply test if knowing within `X` degrees of accuracy is viable.  
...
### Divvy Data  
  ...
  Here I simply utilize the values of the divvy data lagged at 7 days. As with the weather data, this is used to get the general movement in the data, and to make prediction of divvy data (at 7 days in the future) easier.
```{r}

divvy_vars_lag7 <-
  remove_prcp_vars %>% 
  select(matches("_cus$|_dep$|sub$")
         ) %>% 
...
# str(divvy_vars_lag7)
# View(divvy_vars_lag7 %>% head(1000))
...
add_divvy_vars_lag7 <-
  ...
  bind_cols(divvy_vars_lag7)

dim(remove_prcp_vars)
dim(add_divvy_vars_lag7)

str(add_divvy_vars_lag7)
View(add_divvy_vars_lag7 %>% head(1000))


## Save the data to the proper folder
saveRDS(add_divvy_vars_lag7,
        paste0(wd,
               "/Data/Interim/",
               "add_divvy_vars_lag7.Rds"
               )
        )


rm(divvy_vars_lag7, remove_prcp_vars)
...
test <-
  func_train_test_indicator(data = data_no_missing,
                            date_var = el_date,
                            train_val_pct = 0.8,
                            train_pct = 0.7,
                            new_col_suffix = "all"
...
colnames(data_no_missing) %>% tail(5)
colnames(test) %>% tail(5)
str(test)
test %>% 
  group_by(data_use_all_data) %>% 
...
View(test_group %>% select(el_date, el_stop_id, data_use_all, data_use_el_stop_id) %>% head(5000))
...
min_train <- min(all_data_datesformatted_list$train$train_date)
max_train <- max(all_data_datesformatted_list$train$train_date)

min_test <- min(all_data_datesformatted_list$test$test_date)
max_test <- max(all_data_datesformatted_list$test$test_date)

pt7_train_days <- round(0.7 * (max_train - min_train))
train_split_point <- min_train + pt7_train_days


dates_dt <- 
  as.data.table(x = as_date(min_train:max_test)
                ) %>%  
  tk_augment_timeseries_signature()
  

colnames(dates_dt)[1] <- "date_"


dates_dt <- dates_dt %>% 
  mutate(yrmth = floor_date(date_, unit = "month"),
         dataset_purpose = case_when(date_ < train_split_point ~ "train",
                                     between(x = date_,
                                             lower = train_split_point,
                                             upper = max_train,
                                             incbounds = TRUE
                                             ) ~ "validation",
                                     TRUE ~ "test"
...
```
  ...
  Loading the final dataset again.
```{r}

##  THINK ABOUT:

  ## Lagging all divvy data by 21 days (including all "date"-related variables)

  ## Removing data on `prcp` & `snow` (as we won't know that accurately in advance)
  ## Modifying `tmax` & `tmin` to 10 degree F increments (as we're more likely to know that in advance than specific temperature readings)
...
common_el_stop <-
  add_weather %>% 
  group_by(el_stop_id) %>% 
  summarise(rides_tot = sum(el_rides)
            ) %>% 
  top_n(n = 1, wt = rides_tot) %>% 
  pull(el_stop_id)

add_weather %>% 
  filter(el_stop_id == common_el_stop) %>% 
  ggplot(aes(x = el_date,
             y = el_rides
             )
         ) +
...
## Just inspecting the data to this point
```{r}
